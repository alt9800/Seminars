...
以下のようになっている場所の後`map.on('load', function () {` に中心点を生成し、そこからバッファを作成します。
...
```
このような書き方をすることで、無名の関数として、実行したい処理を直後に書くことができて便利です。

もっと省略すると`() => {}`のような短い書き方(アロー関数)になります。


---

ここまででピンとくる方もいらっしゃると思いますが、
